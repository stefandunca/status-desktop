...
method init*(self: Controller) =
  ...
  self.events.on(SIGNAL_CHAT_UPDATE) do(e: Args):
    ...
    for chatDto in args.chats:
      if not self.delegate.doesChatExist(chatDto.id):
        self.delegate.addNewChat(chatDto, false, self.events, self.settingsService, self.contactService, self.chatService,
